...
CURRICULUM_STEPS = 60000  # Use random opponent for first 50k steps
...
class MultiAgentRacingEnv(gym.Env):
    ...
    def __init__(self, port, opponent_policy=None):
        ...
        # Progress tracking
        self.main_total_dist = 0.0
        self.opp_total_dist = 0.0
        self.prev_main_pos = None
    ...
    def step(self, main_action):
        ...
        self.main_info['main_progress'] = self.main_total_dist
        self.main_info['opp_progress'] = self.opp_total_dist
        
        return self.main_obs, competitive_reward, done, self.main_info
    
    def _update_distance_tracking(self):
        """Update cumulative forward distance for both cars"""
        main_pos = np.array(self.main_info.get('pos', (0, 0, 0)))
        opp_pos = np.array(self.opp_info.get('pos', (0, 0, 0)))
        
        if self.prev_main_pos is not None:
            # Main car - only count if moving forward
            main_dist = np.linalg.norm(main_pos - self.prev_main_pos)
            if self.main_info.get('forward_vel', 0) > 0.1:
                self.main_total_dist += main_dist
            
            # Opponent car - only count if moving forward
            opp_dist = np.linalg.norm(opp_pos - self.prev_opp_pos)
            if self.opp_info.get('forward_vel', 0) > 0.1:
                self.opp_total_dist += opp_dist
        
        self.prev_main_pos = main_pos
    ...
    def _calculate_competitive_reward(self):
        ...
        # Competitive component based on forward progress
        competitive_bonus = 0.0
        
        progress_diff = self.main_total_dist - self.opp_total_dist
        while steps_done > CURRICULUM_STEPS:
            
            if progress_diff > 0:
                    # Ahead bonus
                competitive_bonus += 1.5
                    # Bigger lead = more bonus (capped)
                competitive_bonus += min(progress_diff * 0.5, 3.5)
            else:
                    # Behind penalty
        ...
        return base_reward + competitive_bonus
    ...
    try:
        ...
        while steps_done < TOTAL_STEPS:
            try:
                ...
                try:
                    base_env = train_env.venv.envs[0]
